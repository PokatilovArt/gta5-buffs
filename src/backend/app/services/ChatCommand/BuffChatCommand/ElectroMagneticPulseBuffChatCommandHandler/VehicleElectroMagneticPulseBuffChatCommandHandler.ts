...
@BuffChatCommandHandler(BuffChatCommandType.VehicleElectroMagneticPulse)
export class VehicleElectroMagneticPulseBuffChatCommandHandler extends IBuffChatCommandHandler {
  constructor(
    @Inject(BuffService)
    ...
    switch (parsedArgs.state) {
      case BuffChatCommandStateType.On:
        this.buffService.applyBuff({
          entity,
          buffType: BuffType.ElectroMagneticPulse,
          source: player,
        });
        break;

      case BuffChatCommandStateType.Off:
        this.buffService.removeBuff(entity, BuffType.ElectroMagneticPulse);
        break;
    }
  }
}
